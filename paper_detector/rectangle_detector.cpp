		...
		if (configPaperDetectorMinRectangleArea < detectedRectangle.contourArea && detectedRectangle.contourArea < configPaperDetectorMaxRectangleArea) {
			...
			if (detectedRectangle.huDiff / detectedRectangle.boundingRectArea < configPaperDetectorMaxShapeHuDiff) {
				if (numberOfRects < max_rects) {
					...
					if (plotImage != 0) {
						char buf[200];
						sprintf(buf,"%.3f",detectedRectangle.epsilon);
						drawContours(*plotImage,contours,k,Scalar(0,200,200,128),2);
						putText(*plotImage,buf,rotRect.center,FONT_HERSHEY_PLAIN,1.0,Scalar(0,0,0,200),3);
						putText(*plotImage,buf,rotRect.center,FONT_HERSHEY_PLAIN,1.0,Scalar(200,200,200,200),1);
					}
			...
			if (plotImage != 0) {
				drawContours(*plotImage,contours,k,Scalar(0,130,130,64),3);
			}
		}
	}
